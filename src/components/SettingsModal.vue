  ...
  <div class="max-w-2xl p-4 mx-auto">
    ...
    >
      ...
      <div class="flex items-start">
        <div class="pt-6 sticky top-0 px-[20px]" style="background:#fff!important">
          <div @click="changeTab1" :class="!openTab?'dct-active-tab':''" class="dct-drawer-tab">
            <p class="flex items-center gap-2.5">
              <svg viewBox="0 0 24 24" class="w-[28px] h-[28px]" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="dct-drawer-main-section w-full h-fit bg-[#fff] z-10"
...
Здесь можно выбрать услугу которую вы оказываете и при переходе в этап, если поле будет заполнено и будет выбран этот параметр, событие будет создано именно в этом календаре. "
              ...
              >
                <template #popover>
                  <p>
                    ...
                    Здесь можно выбрать услугу которую вы оказываете и при переходе в этап, если поле будет заполнено и
                    будет выбран этот параметр, событие будет создано именно в этом календаре.
                  </p>
                  <p class="pt-4">
                    Если все сотрудники пользуются одним календарем, чтобы не было путаницы необходимо сделать
                    сопоставление поля (например имя сотрудника) и внутреннего календаря. В таком случае можно будет
                    быстро просмотреть все занятые слоты, либо просмотреть только одного сотрудника или услугу.
                  </p>
                </template>
              </Select>
              ...
              <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white flex">
                ...
                <Popover>
                  ...
                  <template #context>
                    <ul>
                      <li class="pt-4">
                        <b style="font-weight: 800">Вариант 1.</b> Вы вручную для каждой сделки определяете дату и время
                        начала и завершения события, посредством заполнения полей amoCRM.
                        Например: 20.06.2023 15:00 - 20.06.2023 17:30
                      </li>
                      <li class="pt-4">
                        <b style="font-weight: 800">Вариант 2.</b> Вы назначаете дату и время начала события, а
                        продолжительность уже будет фиксирована для всех событий.
                        Например: Начало события 20.06.2023 15:00, а длительность всех событий будет по умолчанию 1
                        час."
                      </li>
                    </ul>
                  </template>
              ...
              <div class="">
                ...
                <div class="flex gap-2 justify-between items-end">
                  ...
                  <div class="flex flex-col">
                    <label class="dct-label -mb-1.5 text-sm text-gray-900 dark:text-white">Укажите длительность</label>
            ...
            <!-- card 3 -->
            <div
                class="w-full border border-gray-200 rounded-lg py-3 dark:bg-gray-800 dark:border-gray-700"
            >
              <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">
                Настройки Контента
              </h2>
              ...
              >
                <template #popover>
                  <p>Выбираете поле в amoCRM, которое будет отображать название события в Google Календаре. Можете
                    выбрать название сделки, для того чтобы событие и сделка назывались одинаково</p>
                </template>
              </Select>
              <Select
              ...
              >
                <template #popover>
                  <p>Выбираете поле в amoCRM, где вы прописываете адрес и после создания события в Google Календаре,
                    можно будет посмотреть адрес отдельно и даже открыть его на Google Maps</p>
                </template>
              </Select>
              <div class="my-2 relative">
                <div class="flex">
                  ...
                  <Popover>
                    ...
                    <template #context>
                      Это развернутое описание события в Google Календаре,<br> которое позволит передать любую
                      необходимую информацию из полей amoCRM используя текст и переменные (все переменные доступны по
                      кнопке “Открыть Шаблонизатор”
                    </template>
                  </Popover>
                </div>
                <textarea
                    id="description"
                    rows="8"
                    class="dct-textarea-v2 block p-2.5 w-full text-sm text-gray-900 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                ...
                <button
                    ...
                    type="button"
                    class="dct-button-absolute absolute text-white bg-[#2a7cef] hover:bg-[#5c8bf9] focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-[#2a7cef] dark:hover:bg-[#2a7cef] dark:focus:ring-blue-800"
                >
          ...
          <div v-if="openTab" class="w-full h-screen ml-0"
               style="margin-left:0px!important;background: #ffffff!important">
            <CalendarSettings/>
          </div>
        </div>
      </div>
      <div class="flex justify-start sticky bottom-0 bg-white px-[20px] py-[13px] w-full z-20 gap-2"
        ...
        <button
            @click="closeModal"
            type="button"
            class="text-gray-500 border-[2px] border-solid border-[#efefef] bg-white focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:focus:ring-gray-600"
        >
          Закрыть
        </button>
      </div>
      <div v-if="calendarSettingsStore.loader" class="absolute w-full h-full top-0 left-0 bg-[#f5f5f5f5] flex justify-center items-center pointer-events-none"
...
const {fields, statuses, calendars, selects, markers} = storeToRefs(selectStore);
...
onMounted(async () => {
  ...
  console.log(settings.value.services_parent_id, settingsStore.settings.google_account_id)
  services.value = []
  if (settings.value.services_parent_id) {
    services.value = getServices(settings.value.services_parent_id);
  }
  await calendarSettingsStore.getCalendars(settingsStore.settings.google_account_id)
